...
def baixar_encartes(jornal_num, download_dir):
    links_download = wait.until(
        EC.presence_of_all_elements_located((By.XPATH, "//a[contains(@class, 'download') and contains(@href, '.jpeg')]")))
    ...
    for estado, loja in LOJAS_ESTADOS.items():
        ...
        for i in range(2, 6):
            try:
                ...
                time.sleep(5)
                pass_button = (f"//button.slick-next slick-arrow", By.XPATH)
                pass_button(clicar_elemento)
                time.sleep(2)
                baixar_encartes(1, download_dir)
            except:
                print("Error in process to pass other slide")
